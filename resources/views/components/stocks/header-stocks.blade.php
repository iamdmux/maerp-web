    ...
    <div class="w-full">
        <div class="container mx-auto">
            ...
            <div class="py-1 flex flex-row items-center justify-between">
                <a href="{{route('stocks.home', app()->getLocale())}}" class="text-lg font-semibold ">
            ...
            <nav :class="{'flex': open, 'hidden': !open}" class="flex-col flex-grow px-2 md:px-0 pb-4 md:pb-0  hidden md:flex md:justify-end md:flex-row">
                @php
                    $menu_a_class = "w-full md:w-auto text-center md:text-left self-start px-4 py-2 mt-2 mx-1 text-sm font-semibold bg-transparent rounded-lg  hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                @endphp

                <a class="{{$menu_a_class}} {{url()->current() == route('stocks.home', app()->getLocale()) ? 'bg-gray-100' : ''}}" href="{{route('stocks.home', app()->getLocale())}}">{{__('pages.Home')}}</a>
                <a class="{{$menu_a_class}} {{url()->current() == route('stocks.index', app()->getLocale()) ? 'bg-gray-100' : ''}}" href="{{route('stocks.index', app()->getLocale())}}">{{__('pages.Stocks')}}</a>
                <a class="{{$menu_a_class}} {{url()->current() == route('stocks.company', app()->getLocale()) ? 'bg-gray-100' : ''}}" href="{{route('stocks.company', app()->getLocale())}}">{{__('pages.About Us')}}</a>
                <a class="{{$menu_a_class}} {{url()->current() == route('stocks.contacts', app()->getLocale()) ? 'bg-gray-1d00' : ''}}" href="{{route('stocks.contacts', app()->getLocale())}}">{{__('pages.Contacts')}}</a>
                
                

                @guest
                <a class="{{$menu_a_class}} text-gray-500" href="/register">{{ __('Register')}}</a>
                <a class="{{$menu_a_class}} text-gray-500" href="/login">{{ __('Login')}}</a>
                @endguest
                    ...
                    <button @click="open = !open" class="{{$menu_a_class}}">
                        ...
                        class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                            ...
                            <div class="px-2 py-2 text-center bg-white rounded-md shadow">
                                ...
                                <a class="{{$dropDownClass}}" href="{{route('orders.index', app()->getLocale())}}">{{ __('pages.My orders')}}</a>
                                @can('erp user')
                                ...
                                <a class="relative {{$dropDownClass}}" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                                    {{ __('pages.Logout')}}
                                    <svg class="w-4 h-4 absolute right-0 top-0 mt-3 mr-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                ...
                {{-- cart --}}
                @auth
                <a class="{{$menu_a_class}}" href="{{route('cart.index', app()->getLocale())}}">
                ...
                <div @click.away="open = false" class="relative z-50" x-data="{ open: false }">
                    ...
                    {{-- dropdown list--}}
                    <div x-show="open"  style="display:none" class="absolute bg-white right-0 w-full mt-2 origin-top-right rounded-md shadow-lg text-center">
                        @foreach ($lang as $l => $flag)
                            <a class="{{$dropDownClass}}" href="{{ help_replace_url_with_locale( url()->current(), $l) }}">{{$flag}}</a>
                        @endforeach
                    </div>
                </div>
            </nav>
